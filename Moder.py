...
servers = 888917788058587186 #server ids
...
class NotInRangeError(Exception):
    def __init__(self, salary, message="Id сервера не входит в диапазон (10, 100000000000000000000)"):
            self.salary = salary
            self.message = message
            
            super().__init__(self.message)
if not 5000 < servers < 100000000000000000000:
    raise NotInRangeError(servers)
...
async def aban(ctx, member: Option(discord.Member, description = "Who do you want to ban?"), reason: Option(str, description = "Why?", required = False)):
...
async def pasha(ctx):
    ...
    a = 0
    while a<5:
        await ctx.send(f"<@726833252601430038>")
        a += 1
    city = random.choice([" - Цыган"," - 10 рублей", " - 50 рублей", " - 100 рублей", " - натс",  " - KFC"])
    await ctx.send(city)
    with open('logs\log.txt', 'a') as log:
        log.write(f'{time_string} <@{ctx.author.id}> tagged Pasha\n')
...
async def mod(ctx, member: discord.Member):
    ...
    if ctx.author.id in arr:
        ...
        role_get = get(member.guild.roles, id=1102587572343025825) 
    ...
    with open('users\DeTest.txt', 'r' ,encoding='utf-8') as f:
        ...
        if f'{member.id}' in arr:
            print (member.id, ' ', arr)
            await ctx.respond(f' <@{member.id}>, Вы не будете внесены в реестр уровня Тестер')
        else:
...
async def depass(ctx, member: discord.Member):
    ...
    if ctx.author.id in arr:
        ...
        role_get = get(member.guild.roles, id=888918119400222731) 
        await member.remove_roles(role_get)
...
#PING
@bot.slash_command()
async def ping(ctx, member: Option(discord.Member)):
    with open('users\Admins.txt', 'r' ,encoding='utf-8') as f:
        arr = [int(i.rstrip()) for i in f]
        
    if ctx.author.id in arr:
        await ctx.respond(f"Well done <@{member.id}>")
    else:
        await ctx.respond(embed=embed)
        
    with open('logs\log.txt', 'a') as log:
        log.write(f"{time_string} <@{ctx.author.id}> ping <@{member.id}>\n")
...
async def ad(ctx):
    a = 0
    while a < 1000:
        await ctx.respond("<@726833252601430038> ты пидор!")
        time.sleep(3600)
        a += 1

bot.run('MTA1NzM5MzU4Nzc5ODY4Mzc4MA.Gq5eE3.wfEPHcF2IS3KgygfwQ8e-aog_nevEvdZcU7bRQ') #token: KEEP THIS SAFE, from your developer portal